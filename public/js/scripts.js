...
document.addEventListener(
  ...
  (function () {
    ...
    return function (e) {
      ...
      image.setAttribute(
        "sizes",
        e.target.getAttribute("data-sizes") || e.target.offsetWidth + "px"
      );
...
(function ($) {
  ...
  Berserk.behaviors.fancybox_init = {
    attach: function (context, settings) {
      if (
        $(".fancybox:not(.rendered), .fancybox-media:not(.rendered)").length < 1
      ) {
        return;
      }
  ...
  function brk_lazyload() {
    $.each(lazyUrls, function (i, v) {
      $(
        '[data-brk-library*="' + i + '"]:not(.lazyloaded):not(.lazyload)'
      ).addClass("lazyload");
    });
  }
        ...
        if (typeof lazyUrls[required_libs[j]].js !== "undefined") {
          for (var i in lazyUrls[required_libs[j]].js) {
            if (!uniqueLazyUrls[lazyUrls[required_libs[j]].js[i]]) {
              ...
              prefix =
                Berserk.settings.project_prefix &&
                lazyUrls[required_libs[j]].js[i].indexOf("//") === -1
                  ? Berserk.settings.project_prefix
                  : "";
              console.log(
                "JS added: " + prefix + lazyUrls[required_libs[j]].js[i]
              );
        ...
        if (typeof lazyUrls[required_libs[j]].css !== "undefined") {
          for (i in lazyUrls[required_libs[j]].css) {
            if (!uniqueLazyUrls[lazyUrls[required_libs[j]].css[i]]) {
              ...
              prefix =
                Berserk.settings.project_prefix &&
                lazyUrls[required_libs[j]].css[i].indexOf("//") === -1
                  ? Berserk.settings.project_prefix
                  : "";
              console.log(
                "CSS added: " + prefix + lazyUrls[required_libs[j]].css[i]
              );
              $("head").append(
  ...
  */
  ...
  function brs_add_libraries_enqueue(library) {
    ...
    if (typeof libraries[library].dependency !== "undefined") {
      for (i in libraries[library].dependency) {
        if (
          typeof brk_added_libraries[libraries[library].dependency[i]] ===
          "undefined"
        ) {
  ...
  function brs_add_libraries() {
    ...
    if (Object.keys(adds.css).length) {
      ...
      for (i in adds.css) {
        prefix =
          Berserk.settings.project_prefix && adds.css[i].indexOf("//") === -1
            ? Berserk.settings.project_prefix
            : "";
    ...
    if (Object.keys(adds.js).length) {
      ...
      for (i in adds.js) {
        prefix =
          Berserk.settings.project_prefix && adds.js[i].indexOf("//") === -1
            ? Berserk.settings.project_prefix
            : "";
              ...
              $(window).on("scroll", function () {
                ...
                if (scrolled > 0) {
                  if (scrolled > scrollPrev) {
                    ...
                    if (
                      scrolled <
                      header_one.height() + header_one.offset().top
                    ) {
                ...
                if ($linkHref[0] === "#") {
                  $this.on("click", function (e) {
                    ...
                    if ($($linkHref).length) {
                      ...
                      $(".brk-header-mobile-wrap-layer").removeClass(
                        "is-active"
                      );
                    }
                  });
...
(function ($) {
  ...
  Berserk.behaviors.media_embeds_init = {
    attach: function (context, settings) {
      ...
      var hostedVideo = $(".brk-hosted-video:not(.rendered)", context).addClass(
        "rendered"
      );
      ...
      footerWraps.each(function () {
        if ($(this).hasClass("brk-footer__wrapper_animated")) {
          ...
          $(this).on("DOMSubtreeModified", function () {
            if (
              wrapper.outerHeight(true) !== footer.outerHeight(true) &&
...
(function ($) {
  ...
  Berserk.behaviors.progressbar_init = {
    attach: function (context, settings) {
      ...
      var progressBar = $(".progress__bar:not(.rendered)", context).addClass(
        "rendered"
      );
      ...
      // Curve progress bar
      var p_curve = $(".progress__curve:not(.rendered)", context).addClass(
        "rendered"
      );
      if (p_curve.length) {
        $(window).on("resize", function () {
      ...
      // Count progress bar
      var p_count = $(".progress__count:not(.rendered)", context).addClass(
        "rendered"
      );
      if (p_count.length) {
        $(window).on("scroll", function () {
              ...
              if (!it.hasClass("init")) {
                ...
                if (it.parents(".counter__wrapper-filled").length) {
                  countCircle.css({
                    ...
                    transition:
                      "stroke-dashoffset " + countDur / 1000 + "s ease",
                  });
                }
                if (it.parents(".counter__wrapper-filled").length) {
                  countTri.css({
...
(function ($) {
  ...
  Berserk.behaviors.info_box_init = {
    attach: function (context, settings) {
      ...
      if (
        $(context).parent().find(".info-box__wrapper-icon:not(.rendered)")
          .length
      ) {
